import icons from 'url:../img/icons.svg'; //use url for files which are non code file
...
const showRecipe = async function () {
  try {
    ...
    console.log(id);
    if (!id) return;
    renderSpinner(recipeContainer); //adding spinner
    const res = await fetch(
      `https://forkify-api.herokuapp.com/api/v2/recipes/${id}`
    );
    const recipedata = await res.json();
    // console.log(res);
    let { recipe } = recipedata.data; //destructing
  ...
  <div class="recipe__directions">
    ...
    >
     ...
     `;
    recipeContainer.innerHTML = ''; //before adding markup we need to clear old html
